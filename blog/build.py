...
"""Render the Japanese blog: blog/_posts/*.md -> blog/index.html + blog/<slug>/index.html.
    ...
    python3 blog/build.py            # write the pages, sitemap.xml and llms.txt
...
def llms_entries(posts):
    return '\n'.join(f'- [{p["title"]}]({BASE}{p["url"]}): {p["description"]}' for p in posts)
...
def splice(path, block, indent=''):
    """Replace the marked region of a hand-maintained file with generated entries."""
    begin, end = f'{indent}{BEGIN}', f'{indent}{END}'
    text = path.read_text(encoding='utf-8')
    if begin not in text or end not in text:
        die(f'{path.name} needs the "{BEGIN}" / "{END}" markers')
...
def main():
    ...
    posts = load_posts()
    outputs = {BLOG / 'index.html': render_index(posts),
               SITEMAP: splice(SITEMAP, sitemap_entries(posts), indent='  '),
               LLMS: splice(LLMS, llms_entries(posts))}
    for post in posts:
